    ...
    <div>
        <v-dialog max-width="400" v-model="choose">
            <v-card color="#2D2C30;" class="py-5">
                ...
                <v-card class="rounded-lg mx-5 pa-0 my-2" v-for="(item, i) in wallets" :key="i"
...
export default {
    data() {
        return {
            wallets: [
                { icon: 'meta-mask.svg', title: 'Martian' },
                { icon: 'meta-mask.svg', title: 'Petra' }
            ]
        }
    },
    computed: {
    ...
    methods: {
        async selectWallet(item) {
            ...
            this.$store.commit('wallet/chooseWallet', false)
